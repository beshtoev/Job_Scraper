...
| `triage_agent.py` | Codex API fit-scoring agent. Run by `triage.yml`. |
...
- **`triage.yml`** scores new roles via Codex API nightly. Disabled by default — requires `ANTHROPIC_API_KEY`, `CANDIDATE_PROFILE`, `CANDIDATE_RESUME` secrets.
